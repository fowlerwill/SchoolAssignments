...
public class InOrderTreeIterator<keytype,datatype> extends BinaryTreeIterator<keytype,datatype>
{
	...
	{
		if(node != null) {		
			//to do this in order - recurse as far down left as we can first.
			runRecursiveInorder(node.getLeftChild());
			//now that we're as far down left as we can, with a full stack 
			nodes.add(node);
			//make sure to get the right children too.
